...
-  **failsafe\_timeout**: per-request timeout (in seconds) for the ``POST /failsafe`` REST API calls executed when :ref:`DCS Failsafe Mode <dcs_failsafe_mode>` is activated. Useful when round trips between cluster members take longer than the default 2 seconds. Patroni caps the effective value at run time so that the rule ``loop_wait + 2*retry_timeout + 2*failsafe_timeout <= ttl`` is respected, and never uses a value lower than the default. Defaults to ``2``.
-  **postgresql**:
